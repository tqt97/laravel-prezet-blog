...
<div x-data="{
    ...
    <!-- Backdrop -->
    <div x-cloak x-show="open" x-trap.inert.noscroll="open" x-transition:enter="transition duration-300 ease-out"
        x-transition:enter-start="opacity-0" x-transition:enter-end="opacity-100"
        x-transition:leave="transition duration-200 ease-in" x-transition:leave-start="opacity-100"
        x-transition:leave-end="opacity-0" x-bind:aria-hidden="!open"
        ...
        <div x-cloak x-show="open" x-transition:enter="transition duration-300 ease-out"
            x-transition:enter-start="-translate-y-32 opacity-0" x-transition:enter-end="translate-y-0 opacity-100"
            x-transition:leave="transition duration-150 ease-in" x-transition:leave-start="translate-y-0 opacity-100"
            x-transition:leave-end="translate-y-32 opacity-0" x-on:click.outside="closeCommandPalette()"
            ...
            <div class="relative rounded-t-lg bg-white px-2 pt-2 dark:bg-zinc-800">
                <div class="flex w-full items-center rounded-lg bg-zinc-100 px-3 dark:bg-zinc-700/75">
                    ...
                    <input x-ref="elFilter" x-model="filterTerm" x-on:input.debounce.300ms="filter($event)"
                        ...
                        placeholder="{{ __('Search...') }}" tabindex="0" role="combobox" aria-expanded="true"
            ...
            <ul x-show="filterResults.length > 0" x-ref="elListbox" x-on:mousemove.throttle="enableMouseInteraction()"
                ...
                class="max-h-72 overflow-auto rounded-b-xl bg-white p-2 dark:bg-zinc-800" role="listbox">
